...
try:
    ...
    if client is None:
        st.sidebar.markdown('<div class="status-error">❌ Failed to create GCS client</div>', unsafe_allow_html=True)
...
if gcs_status == "connected":
    ...
    try:
        ...
        client = get_gcs_client()
        if client is None:
            st.error("Failed to create GCS client. Check your credentials.")
            st.stop()
        bucket = client.bucket(bucket_name)
